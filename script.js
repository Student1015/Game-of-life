    ...
    for (var i = 0; i < size2; i++) {
        for (var j = 0; j < size2; j++) {
            if (matrix[j][i] instanceof Empty) {
                ...
                fill('yellow')
                matrix[j][i].move()
            } else if (matrix[j][i] instanceof Red) {
                fill("red")
                matrix[j][i].move()
            }
...
class Red extends Item {
    constructor(x, y) {
        super(x, y)
        this.energy = red_energy;
        this.directions = [
            [this.x - 1, this.y - 1],
            [this.x, this.y - 1],
            [this.x + 1, this.y - 1],
    ...
    move() {
        ...
        matrix[this.y][this.x] = new Empty(this.x, this.y)
        if (this.energy > 0) {
            matrix[y][x] = new Red(x, y, this.energy)
            if (Math.random() < 0.05) {
                matrix[yClone][xClone] = new Red(xClone, yClone, this.energy)
            }
        }
    }
}
